...
b, o, i, g, degree, σ = 100, 8, 7, 2, 2, [one(Float32)]
...
function test_B_spline_basis()
    ...
    recovered_coef = curve2coef(basis_function, x_eval, y, extended_grid, σ; init = true, ε = 1.0f-4)
...
function test_RBF_basis()
    ...
    basis_function = RBF_basis(scale, i, o, g)

    y = coef2curve_Spline(basis_function, x_eval, grid, coef, σ)
    @test size(y) == (i, o, b)
    @test !any(isnan.(y))

    recovered_coef = curve2coef(basis_function, x_eval, y, grid, σ; init = true, ε = 1.0f-4)
    @test size(recovered_coef) == size(coef)
